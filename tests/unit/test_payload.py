...
class TestPayloadGeneration:
    ...
    def test_create_jsonl_payload_with_file_path(self, mock_gcp_clients):
        ...
        try:
            ...
            job._upload_file_to_gcs = Mock(return_value=("gs://test-bucket/test-file.txt", "text/plain"))
            ...
            assert file_part["file_data"]["file_uri"] == "gs://test-bucket/test-file.txt"
    ...
    def test_create_jsonl_payload_with_bytes_input(self, mock_gcp_clients):
        ...
        # Mock the file upload
        job._upload_file_to_gcs = Mock(return_value=("gs://test-bucket/file_bytes", "application/octet-stream"))
    ...
    def test_create_jsonl_payload_schema_serialization(self, mock_gcp_clients):
        ...
        assert function_decl["description"] == "Extracts structured information based on the schema."
...
class TestFileUpload:
    ...
    def test_upload_file_to_gcs_with_string_path(self, mock_gcp_clients):
        ...
        try:
            gcs_uri, mime_type = job._upload_file_to_gcs(temp_file_path, "test/path.txt")
            
            assert gcs_uri == "gs://pyrtex-assets-test-project/test/path.txt"
            assert mime_type == "text/plain"
            
            # Verify mock calls
            mock_gcp_clients['storage'].bucket.assert_called_with("pyrtex-assets-test-project")
    ...
    def test_upload_file_to_gcs_with_path_object(self, mock_gcp_clients):
        ...
        try:
            gcs_uri, mime_type = job._upload_file_to_gcs(temp_file_path, "test/data.json")
