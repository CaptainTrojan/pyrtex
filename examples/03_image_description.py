...
def main():
    ...
    job.add_request("invoice_extraction", PDFInvoiceInput(
    ...
    for result in results:
        if result.was_successful:
            ...
            print(f"Average item value: ${sum(item.total for item in invoice.items) / len(invoice.items):.2f}")
            print(f"Effective tax rate: {(invoice.tax_amount / invoice.subtotal * 100):.2f}%")
        else:
